...
#include "qhyccdstruct_21-10-12_matlab.h"
#include "stdint.h"
#include "config.h"
//#include <functional>
//#include <string>
...
//Get the number of triger mode for camera // NOT REALLY IN LIB
//EXPORTC uint32_t STDCALL GetQHYCCDTrigerModeNumber(qhyccd_handle *handle, uint32_t *modeNumber);
//Get the name of for every triger mode // NOT REALLY IN LIB
...
// EXPORTFUNC void RegisterPnpEventIn( void (*in_pnp_event_in_func)(char *id)); // NOT REALLY IN LIB

// EXPORTFUNC void RegisterPnpEventOut( void (*in_pnp_event_out_func)(char *id)); // NOT REALLY IN LIB
...
// EXPORTFUNC void RegisterTransferEventError( void (*transfer_event_error_func)()); // func ptr
...
// EXPORTFUNC int STDCALL QHYCCD_fpga_list(struct fpga_info_list &list); // this is redefined below!
...
/// ----------------------------------
/* not in lib and anyway for calling finction pointers
void call_pnp_event();
